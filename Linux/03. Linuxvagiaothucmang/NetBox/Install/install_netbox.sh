...
# Cải tiến bằng cách Thêm xen kẽ vào hàm main, không yêu cầu người dùng nhập luôn ngay từ đầu. Hàm chạy đến đâu, nhập đến đó
...
# Cải tiến bằng cách chỉ cho phép nhập địa chỉ IP, kiểm tra định dạng địa chỉ IP
...
# Yêu cầu người dùng nhập sẽ là các biến giá trị để gán vào trong hàm, khi nào gọi hàm sẽ gán các biến giá trị vào
...
# Có thể thêm các điều kiện if elif để trỏ đến các phiên bản mà người dùng muốn cài đặt.
...
# Nên tạo hàm main để chạy các hàm, mỗi khi chạy hàm có thể in ra một thông báo quá trình, các yêu cầu nhập vào của người dùng hàm chạy đến đâu nhập vào đến đó, không nên chạy liên tục các hàm mà không có thông báo gì gây khó hiểu
# Run the functions
install-sql
create-user-sql
install-redis
